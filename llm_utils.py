...
Job 2: {{"id": 2, "dep": 1, "name": job_name, "description": job_description, "input_resources": "example1.ext, example2.ext", "output_resources": "example1.ext, example2.ext"}}
Job 3: {{"id": 3, "dep": 2, "name": job_name, "description": job_description, "input_resources": "example1.ext, example2.ext", "output_resources": "example1.ext, example2.ext"}}
...
Job N: {{"id": N, "dep": N-1, "name": job_name, "description": job_description, "input_resources": "example1.ext, example2.ext", "output_resources": "example1.ext, example2.ext"}}
[/PIPELINE]
...
def generate_one_code_generation_prompt(job, previous_messages, user_message):
    # Build the prompt
    prompt =f'''[INST] You are an expert software engineer, your goal is to write a Python script that solves the job of "{job["name"]}". 
The script should use these resources as input : {job["input_resources"]}, perform the following job on it :  "{job["name"]} : {job["description"]}". The execution of your script on the input resources should generate these resources : {job["output_resources"]}. 
Don't forget to import the necessary libraries at the beggining of the script.
